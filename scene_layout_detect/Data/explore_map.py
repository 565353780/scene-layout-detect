...
class ExploreMap(object):
    ...
    def __init__(self, unit_size=0.1, free_width=5, floor_height_max=0.1):
    ...
    def updateMap(self, point_array):
        if point_array.shape[0] == 0:
            return True

        min_z_value = np.min(point_array[:, 2])
    ...
    def updateObstacle(self, point_array, paint_width=3):
        for point in point_array:
            z_value = point[2]
            pixel = self.getPixelFromPoint(point)
            x_start_pixel = max(0, pixel[0] - paint_width)
            x_end_pixel = min(self.map.shape[0], pixel[0] + paint_width)
            y_start_pixel = max(0, pixel[1] - paint_width)
            y_end_pixel = min(self.map.shape[1], pixel[1] + paint_width)
            if z_value - self.floor_z_value < self.floor_height_max:
                continue
            self.map[x_start_pixel:x_end_pixel,
                     y_start_pixel:y_end_pixel] = OBSTACLE_COLOR
        return True

    def updateFree(self, point_array, paint_width=3):
        for point in point_array:
            z_value = point[2]
            pixel = self.getPixelFromPoint(point)
            x_start_pixel = max(0, pixel[0] - paint_width)
            ...
            y_end_pixel = min(self.map.shape[1], pixel[1] + paint_width)
            if z_value - self.floor_z_value < self.floor_height_max:
                block = self.map[x_start_pixel:x_end_pixel,
                                 y_start_pixel:y_end_pixel]
                not_obstacle_idx = np.where(block != OBSTACLE_COLOR)
                block[not_obstacle_idx] = FREE_COLOR
                self.map[x_start_pixel:x_end_pixel,
                         y_start_pixel:y_end_pixel] = block
        return True

    def addPoints(self, point_array, paint_width=3, render=True):
        bound_points = point_array[np.where(
            point_array[:, 0] != float("inf"))[0]]

        self.updateMap(bound_points)
        self.updateObstacle(bound_points, paint_width)
        self.updateFree(bound_points, paint_width)
